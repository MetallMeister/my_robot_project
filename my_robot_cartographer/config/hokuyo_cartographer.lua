...
TRAJECTORY_BUILDER_2D.max_range = 4.5  -- 距離測定の最大範囲（メートル）
TRAJECTORY_BUILDER_2D.missing_data_ray_length = 4.5  -- 欠損データの代わりに使用する距離（メートル）
...
-- (★★★変更点★★★) 全体最適化（ループクロージャ）の頻度を調整
-- この値を大きくすると、重い最適化処理の頻度が下がり、リアルタイム性が向上します
...
-- サブマップとモーションフィルターの設定
-- (★★★変更点★★★) 1つのサブマップを構成するLiDARデータ数を減らす
-- この値を小さくすると、より短い移動で新しいサブマップが作成され、地図の更新が速く見えます
